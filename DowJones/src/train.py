...
def run(fold, model, save):
    ...
    # a numpy array
    # features = [x for x in df_train.columns if x not in ['Unnamed: 0', 'Date']]
    features = ['Price', 'Open', 'High', 'Low', 'Vol', 'prev_change']
    
    
    x_train = df_train[features].values
    y_train = df_train['change_class'].values

    x_valid = df_valid[features].values
    y_valid = df_valid['change_class'].values
    ...
    mae = metrics.f1_score(y_valid, preds)
    print(preds)
    print(y_valid)
