...
def run(train, test, target, name):
    ...
    train_preds.to_excel(f"{config.INFERENCE_OUTPUT}{name}_train_inference.xlsx", index=False)
    test_preds.to_excel(f"{config.INFERENCE_OUTPUT}{name}_test_inference.xlsx", index=False)
